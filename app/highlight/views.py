...
def index(request):
    ...
    return render(request, 'highlight/index.html',params)

def videoListView(request):
    template_name = "highlight/video-list.html"
    ctx = {}
    
    query = request.GET.get('q', '')  # 'q' は検索ボックスの名前です
    if query:
        qs = Highlight.objects.filter(title__icontains=query)
    else:
...
class FormView(TemplateView):
    ...
    def get(self,request):
        return render(request, "highlight/formpage.html",context=self.params)
    ...
    def post(self,request):
        if request.method == "POST":
            ...
            if self.params["form"].is_valid():
                ...
                SERVICE_ACCOUNT_FILE = 'highlight/config.json'
                #jsonファイルを使って認証情報を取得
